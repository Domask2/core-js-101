...
function timeSpanToString(startDate, endDate) {
  ...
  const min = Math.floor(date / 60000) % 60;
  const sec = Math.floor((date / 1000) % 60);
  const milisec = (`00${date}`).slice(-3);
  ...
  return `${addZero(hours)}:${addZero(min)}:${addZero(sec)}.${milisec}`;
}
...
function angleBetweenClockHands(date) {
  ...
  const angleRes = Math.min(angle, 360 - angle);

  return Math.abs((angleRes * Math.PI) / 180);
}
